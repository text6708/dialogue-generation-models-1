...
from transformers.generation_utils import calc_banned_ngram_tokens, calc_banned_bad_words_ids, set_scores_to_inf_for_banned_tokens
...
def LayerNorm(normalized_shape, eps=1e-6, elementwise_affine=True):
...
class MeenaModel(PretrainedMeenaModel):
    def __init__(self, config: MeenaConfig):
        ...
        self.encoder = MeenaEncoder(config, self.shared, self.position)
        self.decoder = MeenaDecoder(config, self.shared, self.position)
...
class MeenaForConditionalGeneration(PretrainedMeenaModel):
    base_model_prefix = "model"
    ...
    def set_encoder_input_ids(self,encoder_input_ids:torch.Tensor):
    ...
    def postprocess_next_token_scores(
        ...
        input_ids = torch.cat((self.encoder_input_ids, input_ids),-1)
